...
describe('Dado que estas en el buscador de eventos', () => {
  describe('Cuando busques por id de evento', () => {
    ...
    it('Entonces debes poder agregar comisiones personalizadas por método de pago', () => {
      ...
      var eventComCard = element(by.id("event-comission-card"))
      eventComCard.clear()
      eventComCard.sendKeys("0." + timestamp)
      ...
      browser.sleep(mediumSleepTime)
      expect(element(by.css('.card-content p')).getText()).toBe("concierto de j balvin")
      expect(element(by.css('.card-action #event-comission-card')).getAttribute('value')).toBe("0." + timestamp)
    });
  });
});
  ...
  describe('Cuando hago un booking del evento', () => {
    describe('Cuando escojo pagar con deposito', () => {
      ...
      it('Entonces hace el calculo y mostra las comisiones y el precio total cuando ', () => {
        ...
        eventId.sendKeys('2')
        browser.sleep(mediumSleepTime)
        var eventComDep = element(by.id("event-comission-deposit"))
        eventComDep.clear()
        eventComDep.sendKeys(timestamp)
        browser.sleep(sleepTime)
        element(by.css('app-events-page .btn-large')).click()
        browser.sleep(mediumSleepTime)
        browser.get('/events/2/book')
        ...
        var comission = +timestamp * quantity
        expect(element(by.css('.e2e-total')).getText()).toBe(String(quantity * price + comission))
        expect(element(by.css('.e2e-total-comission')).getText()).toBe(String(comission))
      });
    })
    describe('Cuando escojo pagar con tarjeta', () => {
      ...
      it('Entonces hace el calculo y mostra las comisiones y el precio total cuando ', () => {
        ...
        element(by.css('app-book-page .btn-large')).click()
        browser.sleep(longSleepTime)
        var comission = +timestamp * quantity * price / 100
        expect(element(by.css('.e2e-total')).getText()).toBe(String(quantity * price + comission))
        expect(element(by.css('.e2e-total-comission')).getText()).toBe(String(comission))
      });
    })
  ...
  describe('Cuando hago un booking del evento', () => {
    ...
    describe('Cuando escojo pagar con tarjeta', () => {
      ...
      it('Entonces hace el calculo y mostra las comisiones y el precio total cuando ', () => {
        ...
        element(by.css('app-book-page .btn-large')).click()
        browser.sleep(longSleepTime)
        var comission = 0.035* quantity * price 
        expect(element(by.css('.e2e-total')).getText()).toBe(String(quantity * price + comission))
        expect(element(by.css('.e2e-total-comission')).getText()).toContain('14')
      });
    })
  })
});